sigora <-
    function(GPSrepo,
        ...
        if (length(intersect(queryList, GPSrepo$origRepo[[2]])) == 0) {
            t1 <- which.max(sapply(idmap, function(x)
                    length(intersect(x, GPSrepo$origRepo[[2]]))
            ))

            t2 <- which.max(sapply(idmap, function(x)
        ...
        ps <- phyper(
            k1[, 2] - 1,
            kN[match(k1[, 1], kN[, 1]), 2],
            sum(kN[, 2]) - kN[match(k1[, 1], kN[, 1]), 2], sum(k1[, 2]),
            lower.tail = FALSE
        )
        ...
        Bonfer <- signif(
            p.adjust(ps, n = length(unique(hh[, 3])), method = "bonfer"),
            digits = 4
        )
